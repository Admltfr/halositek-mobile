...
class PortofolioView extends GetView<PortofolioController> {
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              children: [_topBar(), 18.0.sh, _architectOverview(), 24.0.sh, _tabSwitch(), 22.0.sh, _tabContent()],
            ),
          ),
        ),
      ),
  ...
  Widget _architectOverview() {
    return Obx(
      () => Skeletonizer(
        ...
        child: Column(children: [_profileHeader(), 22.0.sh, _stats(), 22.0.sh, _priceAndChat()]),
      ),
    );
  }
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: AppColors.primaryColor.withValues(alpha: 0.18), width: 4),
                  ),
                  child: ClipOval(
                    child:
                        controller.architectPhoto.value.isNotEmpty
                            ? Image.network(
                              ...
                              fit: BoxFit.cover,
                              errorBuilder: (_, __, ___) => Image.asset(_dummyAvatar, fit: BoxFit.cover),
                            )
      ...
      return InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  isSaved ? AppColors.primaryColor.withValues(alpha: 0.32) : AppColors.primaryColor.withValues(alpha: 0.08),
            ),
  ...
  Widget _stats() {
    return Obx(
      () => Row(
        children: [
          Expanded(child: _statCard(controller.totalProjects.value.toString(), 'PROJECTS')),
          12.0.sw,
          Expanded(child: _statCard(controller.totalAwards.value.toString(), 'AWARDS')),
        ],
      ),
    );
  }
  ...
  Widget _statCard(String value, String label) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: AppColors.shadowSoftColor, blurRadius: 8, offset: Offset(0, 3))],
      ),
      child: Column(
  ...
  Widget _tabSwitch() {
    return Obx(() {
      ...
      return Row(
        children: [
          _tabItem(title: 'Portfolio', isActive: active == 0, onTap: () => controller.setTab(0)),
          _tabItem(title: 'Award', isActive: active == 1, onTap: () => controller.setTab(1)),
        ],
      );
    });
  }

  Widget _tabItem({required String title, required bool isActive, required VoidCallback onTap}) {
    return Expanded(
      child: InkWell(
        onTap: onTap,
        child: Column(
          children: [
            Text(
              title,
              style: AppTypography.bodySmall.copyWith(
                color: isActive ? AppColors.primaryColor : AppColors.textBodyColor,
  ...
  Widget _portfolioGrid() {
    ...
    if (hasError && !hasData) {
      return _errorState(message: controller.portfolioError.value, onRetry: controller.fetchPortfolios);
    }
    ...
    return Skeletonizer(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (_, index) => _portfolioCard(controller.portfolios[index]),
      ),
    );
  }

  Widget _emptyPortfolioState() {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(vertical: 40),
      child: Column(
        children: [
          Icon(Icons.work_outline_rounded, size: 48, color: AppColors.textBodyColor.withValues(alpha: 0.5)),
          ...
          Text(
            'Portofolio yang telah dibuat akan ditampilkan di sini.',
            textAlign: TextAlign.center,
  ...
  Widget _awardGrid() {
    ...
    if (hasError && !hasData) {
      return _errorState(message: controller.awardError.value, onRetry: controller.fetchAwards);
    }

    if (!isLoading && !hasData) {
      return _emptyAwardState();
    }
    ...
    return Skeletonizer(
      ...
      child: GridView.builder(
        ...
        itemBuilder: (_, index) => _awardCard(controller.awards[index]),
      ),
    );
  }

  Widget _emptyAwardState() {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.symmetric(vertical: 40),
      child: Column(
        children: [
          Icon(Icons.emoji_events_outlined, size: 48, color: AppColors.textBodyColor.withValues(alpha: 0.5)),
          const SizedBox(height: 12),
          Text(
            'Belum ada award',
            style: AppTypography.bodyMedium.copyWith(color: AppColors.textBodyColor, fontWeight: FontWeight.w600),
          ),
          const SizedBox(height: 4),
          Text(
            'Award yang telah diperoleh akan ditampilkan di sini.',
            textAlign: TextAlign.center,
            style: AppTypography.bodySmall.copyWith(color: AppColors.textBodyColor.withValues(alpha: 0.7)),
          ),
        ],
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          ...
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: AppTypography.bodySmall.copyWith(color: AppColors.textHeadingColor, fontWeight: FontWeight.w900),
        ),
        2.0.sh,
        Text(
  ...
  Widget _errorState({required String message, required VoidCallback onRetry}) {
    return Column(
      children: [
        Text(message, textAlign: TextAlign.center, style: AppTypography.bodySmall.copyWith(color: AppColors.errorColor)),
